...
public class LeftRightJoinReplacer extends QueryModelVisitorBase<OptimizerException>{
	
	private byte joinIdCounter ;
	private char leftSubQueryName ;
	public LeftRightJoinReplacer() {
		super() ;
		joinIdCounter =0 ;
		leftSubQueryName='A' ;
	}
...
//		System.out.println("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
		System.out.println("BIIIIIIIGGG NOTTTTTTTTTTTTICE LEFT AND RIGHT REPLACER CALLED");
		TupleExpr temp ;
		temp=node.getLeftArg() ;
		node.setLeftArg(node.getRightArg()) ;
		node.setRightArg(temp);
